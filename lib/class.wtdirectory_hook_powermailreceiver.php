...
class tx_wtdirectory_powermailreceiver extends AbstractPlugin {
	...
	function PM_FormWrapMarkerHook($OuterMarkerArray, $subpartArray, $conf, $obj) {
		...
		if ($this->piVars['pm_receiver'] > 0) { // if pm_receiver is set in piVars
			...
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery( // DB query
					'email',
					'tt_address',
					$where_clause = 'tt_address.uid = ' . intval($this->piVars['pm_receiver']) . \TYPO3\CMS\Frontend\ContentObject\ContentObjectRenderer::enableFields('tt_address'),
					$groupBy = '',
					$orderBy = '',
					$limit = 1
			);
			...
			if (GeneralUtility::validEmail($row['email'])) { // if this is a valid email address
	...
	function PM_SubmitEmailHook($subpart, &$maildata, $sessiondata, $markerArray, &$obj) {
		...
		if ($subpart == 'recipient_mail') { // work only if mail to receiver
			...
			if (GeneralUtility::validEmail($rec_array['powermailreceiver'])) { // if this is a valid email address
