...
class tx_wtdirectory_filter_search extends tslib_pibase {
	...
	function main($conf, $piVars, $cObj) {
		...
		foreach ($this->searchfields as $value) { // one loop for every needed searchfield
			...
			$this->markerArray['###WTDIRECTORY_SEARCH_PATH###'] = t3lib_div::getIndpEnv('TYPO3_SITE_URL'); //relative serverpath
			$this->markerArray['###WTDIRECTORY_PID###'] = ($this->cObj->data['pages'] > 0 ? $this->pi_getPidList($this->cObj->data['pages'], $this->cObj->data['recursive']) : ''); // selected pid
			$this->markerArray['###WTDIRECTORY_CAT###'] = ($this->pi_getFFvalue($this->conf, 'cat_join', 'mainconfig') > 0 && $this->pi_getFFvalue($this->conf, 'category', 'mainconfig') != '' ? $this->pi_getFFvalue($this->conf, 'category', 'mainconfig') : ''); // selected categories
