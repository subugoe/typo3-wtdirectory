...
class tx_wtdirectory_filter_search extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	public function main($conf, $piVars, $cObj) {
		...
		$this->tmpl = $this->markerArray = $this->markerArray2 = $this->markerArray3 = $this->outerArray = $this->subpartArray = $this->subpartArray2 = array();
		...
		$this->searchfields = GeneralUtility::trimExplode(',', $this->pi_getFFvalue($this->conf, 'search', 'list'), 1); // take searchfieldlist as an array
		$selectfields = GeneralUtility::trimExplode(',', $this->conf['filter.']['select'], 1);
