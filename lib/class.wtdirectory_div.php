...
class wtdirectory_div extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	public function conditions4DetailLink($row, $what, $conf) {
		if ($conf['morelink_detail.']['condition']) { // if there is an entry in constants
			...
			$check4fields =
					GeneralUtility::trimExplode(',', $conf['morelink_detail.']['condition'], 1); // like array('fax', 'mobile')
	...
	public function setPiVars(&$piVars, $conf) {
		if ($piVars['list'] != 'all' && $conf['filter.']['start'] && count($piVars) == 0) { // if tx_wtdirectory_pi1[list]=all is not set AND filter.start is set in constants AND there are no other piVars
			if (strpos($conf['filter.']['start'], 'shownone') === false) { // startfilter was set
				$tmp_startfilter1 = GeneralUtility::trimExplode(',', $conf['filter.']['start'], 1); // split at comma (result e.g. filter|last_name=a%)
				for ($i = 0; $i < count($tmp_startfilter1); $i++) { // one loop for every filter to set
					$tmp_startfilter2 = GeneralUtility::trimExplode('=', $tmp_startfilter1[$i], 1); // split at = (result e.g. filter|last_name)
					$tmp_startfilter3 = GeneralUtility::trimExplode('|', $tmp_startfilter2[0], 1); // split at | (result e.g. filter)
	...
	public function allowedDetailUID($uid, $pObj) {
		...
		if (!GeneralUtility::inList($this->pi_getPidList($this->cObj->data['pages'], $this->cObj->data['recursive']), $row[0]['pid'])) { // if the pid is not within the startingpath
			return false; // return 0
		}
		...
		if ($this->pi_getFFvalue($this->conf, 'cat_join', 'mainconfig') == 1) { // if catmode == 1 (show only selected categories)
			...
			$allowed_cat = GeneralUtility::trimExplode(',', $this->pi_getFFvalue($this->conf, 'category', 'mainconfig'), 1); // array with allowed categories
	...
	public function getCategories($pObj) {
		...
		// let's go
		$cat = GeneralUtility::trimExplode(',', $this->pi_getFFvalue($this->conf, 'category', 'mainconfig'), 1); // all chosen categories in an array
	...
	public function encodeBase64($img) {
		if (!empty($img) && file_exists($img)) { // if file really exists
			$code = GeneralUtility::getURL($img); // read image
	...
	public function getAllValuesFromField($field, $cObj, $conf) {
		$tree = GeneralUtility::makeInstance('t3lib_queryGenerator'); // make instance for query generator class
		$arr = array();
		$table = 'tt_address';
		...
		if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('static_info_tables', 0) && $this->pi_getFFvalue($conf, 'countryfilter', 'mainconfig')) { // countryfilter only
			$allowedCountries = GeneralUtility::trimExplode(',', $this->pi_getFFvalue($conf, 'countryfilter', 'mainconfig'), 1);
			$where .= ' AND (';
	...
	public function intList($list) {
		$intList = GeneralUtility::intExplode(',', $list, 1);
	...
	public function getCountryFromCountryCode($iso, $pObj) {
		...
		$staticInfoObj = & GeneralUtility::getUserObj('&tx_staticinfotables_pi1');
	...
	public function getCoordinatesFromZip($geocodeurl = 'http://fa-technik.adfc.de/code/opengeodb/PLZ.tab') {
		$arr = array();
		$table = GeneralUtility::getUrl($geocodeurl); // read url
