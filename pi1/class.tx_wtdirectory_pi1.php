...
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'pi1/class.tx_wtdirectory_pi1_list.php'); // load listview class
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'pi1/class.tx_wtdirectory_pi1_detail.php'); // load detailview class
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'pi1/class.tx_wtdirectory_pi1_vcard.php'); // load vcard class
...
class tx_wtdirectory_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	public function main($content, $conf) {
		...
		if (!empty($this->piVars['vCard'])) { // if vCard GET param was set: vcard export
			...
			$this->vCard = GeneralUtility::makeInstance('tx_wtdirectory_pi1_vcard'); // Create new instance for vCard class
				...
				case 1: // Not set: show list view
					...
					if ($this->conf['showView']) {
						switch ($this->conf['showView']) {
							case 'detailView':
								$this->detailView = GeneralUtility::makeInstance('tx_wtdirectory_pi1_detail'); // Create new instance for detail class
								$this->content = $this->detailView->main($this->conf, $this->piVars, $this->cObj); // Detail view
								break;
						}
					}

					$this->listView = GeneralUtility::makeInstance('tx_wtdirectory_pi1_list'); // Create new instance for list class
	...
	private function secure() {
		if (class_exists('tx_wtdoorman_security')) {
			...
			$varsFromNews = GeneralUtility::_GP('tx_ttnews');
