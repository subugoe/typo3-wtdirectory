...
class tx_wtdirectory_pi1_list extends tslib_pibase {
	...
	function main($conf, $piVars, $cObj) {
		...
		if (!$this->pi_getFFvalue($this->conf, 'shownone', 'mainconfig') || count($this->piVars) > 0) { // default mode (show entries at the beginning)
			...
			$this->setFilter();
			if ($this->piVars['pointer'] > $this->overall()) $this->piVars['pointer'] = 0; // Set pointer to 0 if pointer is too high and senseless
		...
		if ($this->conf['debug.']['beforemain.']['sql']==1) 	t3lib_div::debug($GLOBALS['TYPO3_DB']->debug_lastBuiltQuery);
			...
			if ($res) { // If there is a result
				while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) { // One loop for every tt_address entry
					...
					if ($currentAddressGroup <> $row['tt_address_group_title']) {
						...
						$allowedFields[]='tt_address_group_title';
					}
					else {
					...
					if ($this->div->conditions4DetailLink($row, 'list', $this->conf)) {
						...
						if ($linkConf['useGoogleMapLink']==1) $linkConf['additionalParams'] .= '&tx_rggooglemap_pi1[poi]=' . $row['ttaddress_uid'];
					...
					if ($this->pi_getFFvalue($this->conf, 'target', 'powermail')) $this->wrappedSubpartArray['###WTDIRECTORY_POWERMAIL_LINK###'] = $this->cObj->typolinkWrap( array('parameter' => $this->pi_getFFvalue($this->conf, 'target', 'powermail'), 'additionalParams' => '&' . $this->prefixId . '[pm_receiver]=' . $row['ttaddress_uid'], 'useCacheHash' => 1) ); // Link to powermail page
...
#				$this->filter .= ' AND tt_address.uid IN (SELECT tt_address.uid FROM tt_address INNER JOIN tt_address_group_mm ON tt_address.uid = tt_address_group_mm.uid_local WHERE tt_address_group_mm.uid_foreign IN ('. $cats   .'))';
