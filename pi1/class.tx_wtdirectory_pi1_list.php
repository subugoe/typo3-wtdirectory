...
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'lib/class.wtdirectory_filter_abc.php'); // load abc filter class
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'lib/class.wtdirectory_filter_search.php'); // load search filter class
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'lib/class.wtdirectory_filter_cat.php'); // load category filter class
require_once(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('wt_directory') . 'lib/class.wtdirectory_filter_radialsearch.php'); // load radialsearch class
...
class tx_wtdirectory_pi1_list extends AbstractPlugin {
	...
	public function main($conf, $piVars, $cObj) {
		...
		$this->filter_abc = GeneralUtility::makeInstance('tx_wtdirectory_filter_abc'); // Create new instance for abcfilter class
		$this->filter_search = GeneralUtility::makeInstance('tx_wtdirectory_filter_search'); // Create new instance for searchfilter class
		$this->filter_cat = GeneralUtility::makeInstance('tx_wtdirectory_filter_cat'); // Create new instance for catfilter class
		$this->filter_radialsearch = GeneralUtility::makeInstance('tx_wtdirectory_filter_radialsearch'); // Create new instance for radialsearch class
		$this->pagebrowser = GeneralUtility::makeInstance('tx_wtdirectory_pagebrowser'); // Create new instance for pagebrowser class
		...
		if (!$this->pi_getFFvalue($this->conf, 'shownone', 'mainconfig') || count($this->piVars) > 0) { // default mode (show entries at the beginning)
			...
			if ($res) { // If there is a result
				while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) { // One loop for every tt_address entry
					...
					$allowedFields = GeneralUtility::trimExplode(',', $this->pi_getFFvalue($this->conf, 'field', 'list'), 1);
		...
		$tmp_OuterSubpartArray = $this->markers->makeMarkers('list', $this->conf, $row, GeneralUtility::trimExplode(',', $this->pi_getFFvalue($this->conf, 'field', 'list'), 1), $this->piVars); // get markerArray
	...
	private function setFilter() {
		...
		if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('static_info_tables', 0) && $this->pi_getFFvalue($this->conf, 'countryfilter', 'mainconfig')) {
			$countries = GeneralUtility::trimExplode(',', $this->pi_getFFvalue($this->conf, 'countryfilter', 'mainconfig'), 1); // get countries
			$this->filter .= ' AND (';
